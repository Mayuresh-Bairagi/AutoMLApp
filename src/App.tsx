  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        if (data.preview && data.session_id) {
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800">CSV & Excel Previewer</h1>
        <p className="text-gray-500">Upload your spreadsheet files and preview them beautifully</p>
      </div>
        ...
        <AnimatePresence>
          {preview.length > 0 && sessionId && (
            ...
            >
              <button
                onClick={() =>
                  navigate("/preview", { state: { preview, fileName: file?.name, sessionId } })
                }
